...
buscar_pdfs_sharepoint <- function(tenant_id, client_id, client_secret, dominio, sitio, patron) {
	...
	if (!is.null(result$value) && length(result$value) > 0) {
		...
		pdfs_filtrados <- pdfs[grepl("07_Evidencias", pdfs$webUrl, ignore.case = TRUE), ]
		pdfs_filtrados <- pdfs_filtrados[grepl("3_Comfenalco_Antioquia", pdfs$webUrl, ignore.case = TRUE), ]
		cat("✅ Se encontraron ", nrow(pdfs_filtrados), " PDFs.\n")
